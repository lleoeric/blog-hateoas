...
import cn.leo.service.domain.BaseService;
import cn.leo.utils.GenericsUtils;
...
public class BaseServiceImpl<T, ID, R extends JpaRepository<T, ID>> implements BaseService<T, ID> {
    ...
    @Override
    public <S extends T> List<S> findAll(Example<S> example, Pageable pageable) {
        return (List<S>) repository.findAll(example, pageable);
    }
